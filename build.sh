...
usage()
{
  ...
  echo "  bash build.sh [options] [device] [variant] [version]"
  echo ""
...
if [[ -z $version ]]; then
  echo "${blu}WARNING:${txtrst} No version specified, '0' will be used"
  version='0'
  export DARKOBASV=$version
  echo "Darkobas Version: $DARKOBASV "
fi
